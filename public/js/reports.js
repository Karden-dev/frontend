...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const renderReportsTable = (reports) => {
        ...
        reportsToRender.forEach((report, index) => {
            ...
            <td>${report.total_orders_sent}</td>
            <td>${report.total_orders_delivered}</td>
            ...
            <td class="${amountToRemitClass}">${formatAmount(report.amount_to_remit)}</td>
            <td><button class="btn btn-sm btn-info copy-report-btn" data-shop-id="${report.shop_id}" title="Copier le rapport détaillé"><i class="bi bi-clipboard"></i></button></td>`;
    ...
    const updateGlobalTotals = (reports) => {
        ...
        reports.forEach(report => {
            // Marchands Actifs: ceux qui ont envoyé au moins une commande
            ...
            // Total Versements: somme des montants à verser positifs
            if (amountToRemit > 0) {
                totalRemit += amountToRemit;
            }
            // Créances: somme des montants à verser négatifs (en valeur absolue)
    ...
    if (processStorageBtn) {
        processStorageBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                // On rafraîchit les rapports pour afficher les nouveaux frais de stockage
