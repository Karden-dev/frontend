 ...
 * Gère l'affichage des KPIs, la rentabilité et l'interface utilisateur.
 */
...
/* --- 1. GESTION DE L'INTERFACE (Sidebar & User) --- */
...
function initializeUser() {
    // Récupération de l'utilisateur (Simulé ou depuis localStorage)
    const userStr = localStorage.getItem('user_data');
    const user = userStr ? JSON.parse(userStr) : { 
        name: 'Admin Principal', 
        role: 'Gérant',
        email: 'admin@winkshop.com'
    };
    ...
    if (userAvatarEl) {
        // Génération d'avatar automatique avec les initiales si pas d'image
        userAvatarEl.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=4f46e5&color=fff&size=128`;
    }
...
function updateNotifications() {
    ...
    // Exemple de notifications basées sur le PDF (Stock & Rappels)
    const notifications = [
        { type: 'warning', text: 'Stock faible : Écouteurs Pro (reste 2)' },
        { type: 'info', text: 'Rappel : 3 commandes à livrer demain' }
    ];
        ...
        notifList.innerHTML = notifications.map(n => `
            <li class="p-3 border-bottom bg-white">
                <div class="d-flex align-items-center">
                    <i class="fas ${n.type === 'warning' ? 'fa-exclamation-triangle text-warning' : 'fa-clock text-info'} me-3"></i>
                    <div>
                        <div class="small text-dark fw-medium">${n.text}</div>
                        <div class="text-muted" style="font-size: 0.7rem;">Il y a 10 min</div>
                    </div>
                </div>
            </li>
        `).join('');
...
/* --- 2. LOGIQUE MÉTIER (Finance & Commandes) --- */
