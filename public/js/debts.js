...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const renderDebtsTable = (debts) => {
        ...
        if (debts.length === 0) {
            debtsTableBody.innerHTML = `<tr><td colspan="7" class="text-center p-3">Aucune créance à afficher pour les filtres sélectionnés.</td></tr>`;
            return;
        }
                ...
                <td class="text-center">
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <li><a class="dropdown-item edit-btn ${!isManual ? 'disabled' : ''}" href="#" data-id="${debt.id}"><i class="bi bi-pencil"></i> Modifier</a></li>
                            <li><a class="dropdown-item delete-btn text-danger ${!isManual ? 'disabled' : ''}" href="#" data-id="${debt.id}"><i class="bi bi-trash"></i> Supprimer</a></li>
                        </ul>
                    </div>
                </td>
            `;
            ...
            if (confirm("Êtes-vous sûr de vouloir supprimer cette créance manuelle ?")) {
             ...
             if (confirm("Confirmer le règlement de cette créance ?")) {
                try {
                    ...
                    showNotification("Erreur lors du règlement.", "danger");
                }
            }
        }
    });
